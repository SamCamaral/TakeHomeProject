...
export default function QuizContainer() {
  ...
  useEffect(() => {
    ...
    const handleShowQuiz = async (data: RpcData): Promise<string> => {
      try {
    ...
    <AnimatePresence>
      {isVisible && currentQuestion && (
        <motion.div
          ...
          className="fixed left-8 top-1/4 w-80 bg-white p-4 rounded-lg shadow-lg border border-[#E6E1DD]"
          style={{ color: "#28292A" }}
        >
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-bold text-[#28292A]">Quiz</h2>
            <button 
              onClick={() => setIsVisible(false)}
              className="text-[#333333] hover:text-[#28292A] text-2xl leading-none"
            >
              ×
            </button>
          </div>
